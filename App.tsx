...
const App: React.FC = () => {
  ...
  const handleConceptClick = async (term: string, existingDef?: string) => {
    ...
    setPoints(p => p + 2); // Small reward for curiosity
    try {
  ...
  const renderTextWithKeyTerms = (text: string) => {
    ...
    results.keyTerms.forEach(({ term, definition }) => {
  ...
  return (
    <div className="min-h-screen bg-[#fcfdfe] dark:bg-slate-950 flex flex-col font-['Inter'] transition-colors duration-500 overflow-x-hidden">
      <header className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-b border-slate-100 dark:border-slate-800 sticky top-0 z-[60] shadow-sm">
        <div className="max-w-[1440px] mx-auto px-4 lg:px-6 h-16 lg:h-20 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 lg:gap-8 overflow-hidden">
            <div onClick={() => { setResults(null); setQuery(''); }} className="flex items-center gap-2 cursor-pointer shrink-0">
              ...
              <div className="relative flex flex-col lg:flex-row lg:items-center bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-3xl p-4 lg:px-10 lg:py-7 shadow-2xl focus-within:border-indigo-500 transition-all">
                ...
                <div className="flex items-center gap-4 lg:ml-8 lg:pl-8 lg:border-l-2 lg:border-slate-50 dark:lg:border-slate-800">
                  <div className="flex gap-2">
                    <button type="button" onClick={() => setIsListening(!isListening)} className="p-3 text-slate-300 hover:text-rose-500 transition-colors"><Mic size={28} /></button>
            ...
            {activeTab === 'all' && (
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-8 space-y-6">
                  ...
                  <div className="bg-white dark:bg-slate-800 p-8 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-xl shadow-indigo-100/20 dark:shadow-none">
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-8">
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={playAIExplanation} className={`p-3 rounded-full ${isReading ? 'bg-indigo-600 text-white animate-pulse' : 'bg-slate-100 dark:bg-slate-700 text-slate-500 hover:bg-indigo-600 hover:text-white'}`}><Volume2 size={24} /></button>
                      </div>
                    </div>
            ...
            {activeTab === 'images' && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {results.imageResults.map((img, i) => (
                  <div key={i} className="bg-white dark:bg-slate-800 rounded-3xl overflow-hidden shadow-lg group">
      ...
      {/* Floating Concept Breakdown Overlay */}
      {activeBreakdown && (
        <div className="fixed inset-0 z-[100] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-300">
           <div className="bg-white dark:bg-slate-900 w-full max-w-xl rounded-[2.5rem] overflow-hidden shadow-2xl border border-white/20 dark:border-slate-800 relative group animate-in zoom-in-95 duration-300">
              ...
              <div className="p-8 lg:p-10">
                 ...
                 <div className="mt-8 flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-2 text-slate-400 font-bold text-[10px] uppercase tracking-widest">
                       <Zap size={14} className="text-indigo-500" /> Synthesis Verified
                    </div>
                 </div>
              </div>
           </div>
